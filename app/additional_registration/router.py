...
router = APIRouter(
    prefix="/events/additional", tags=["Соты регистрация на ивент (доп места)"]
)
...
async def registration_on_event(
    ...
    new_registration = await RegistrationAddDao.find_one_or_none(
    ...
    in_addreg = await RegistrationDao.find_one_or_none(
        event_id=event_id, user_id=current_user.id
    )
    if in_addreg:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="у вас уже есть запись на основные места",
        )
...
async def disregistration_on_event(
    ...
    is_registration = await RegistrationAddDao.find_one_or_none(
        event_id=event_id, user_id=current_user.id
    )
...
async def my_registration_list(current_user: User = Depends(get_current_user)):
    events = await RegistrationAddDao.find_all(user_id=current_user.id)
    return events
...
async def my_registration(
    ...
    events = await RegistrationAddDao.find_one_or_none(
        user_id=current_user.id, event_id=event_id
    )
    if not events:
        raise HTTPException(
